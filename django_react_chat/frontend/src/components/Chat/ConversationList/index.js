...
import {chat_status, last_seen_time, is_typing, chat_requests, msg_count, notifications, chat_messages, has_read} from '../../../redux'
...
const ConversationList = (props) => {
  ...
  useEffect(() => {
    let c = [];
    let temp = {};
    if(session_chat_requests && session_chat_requests.hasOwnProperty('reqd_chat_data')){
      session_chat_requests['reqd_chat_data'].map(item => {
        if(item.chat.isFriend){
          temp[item.chat.chat_id] = item.chat.author_id?item.chat.author_id === curr_user_data.id?'You: '+item.chat.text:item.chat.text:'Start your conversation ...';
          c.push(item)
        }
      })
    }
    
    if(session_chat_requests && session_chat_requests.hasOwnProperty('last_seen')){
      setLastSeenState(session_chat_requests['last_seen']);
    }
    if(session_chat_requests && session_chat_requests.hasOwnProperty('is_typing')){
      ...
      if(session_chat_messages && session_chat_messages.hasOwnProperty('recent_msg_data') && session_chat_messages['recent_msg_data']){
        if(location.state){
          if(WebSocketInstance.state() === 1){
            ...
            let curr_recent_msg = session_chat_messages['recent_msg_data'].hasOwnProperty(chat_id) && session_chat_messages['recent_msg_data'][chat_id].author_id !== curr_user_data.id?session_chat_messages['recent_msg_data'][chat_id]:{};
        ...
        if(Object.keys(recent_msg).length > 0){
          ...
          Object.keys(recent_msg).map(item => {
            Object.keys(session_chat_messages['recent_msg_data']).map(innerItem => {
              let msg_data = session_chat_messages['recent_msg_data'][innerItem].author_id !== curr_user_data.id?session_chat_messages['recent_msg_data'][innerItem]:{};
              let msg_time = msg_data?new Date(msg_data.timestamp):'';
              let curr_last_seen = msg_data.last_seen && msg_data.last_seen.hasOwnProperty(curr_user_data.id)?msg_data.last_seen[curr_user_data.id]:''
              let curr_last_seen_time = curr_last_seen?new Date(curr_last_seen):'';
              if(item == innerItem){
                if(msg_time > curr_last_seen_time){
                ...
                count_dict[item] = recentMsgCount.hasOwnProperty(item)?recentMsgCount[item]:0
              }
            })
          })
          setRecentMsgCount(count_dict);
  ...
  const setRecentMsgData = (data) => {
    ...
    let recent_msg_content = data['recent_msg_data'][recent_msg_chat_id].content;
    let temp = {};
    ...
    axiosInstance.get('get_recent_msg_data/').then(res => {
      if(res.data.ok) {
        if(res.data && res.data.hasOwnProperty('chats')){
          ...
          chats.map(item => {
            if(item.chat.isFriend){
              recent_msg_d[item.chat.chat_id] = item.chat.author_id?item.chat.author_id === curr_user_data.id?'You: '+item.chat.text:item.chat.text:'Start your conversation ...';
            }
          });
  ...
  const setLastSeenData = (data) => {
    if(data['last_seen_data'].hasOwnProperty(curr_user_data.id)){
      ...
      dispatch(chat_requests(data['last_seen_data'][curr_user_data.id], 'last_seen'));
    }
  }
...
const manageChats = (action, recipient_user, chat_id) => {
 ...
 const getConversations = () => {
  ...
  const setLastSeenMethod = (data) => {
    ...
    axiosInstance.post('set_last_seen/', post_data).then(res => {
      if(res.data.ok) {
            ...
            return ( 
              ...
              <Col style={{ paddingRight: '0%' }} xs={12} sm={12} md={12} lg={12} xl={12}>
                <Row style={{ padding: '2% 0% 2% 0%', margin: '0px' }}>
                  <Col onClick={() => onSelectUser(item.chat)} xs={4} sm={3} md={3} lg={3} xl={3}>
                      <img width="45"
                          height="45" className="img-fluid conversation-photo" src={item.chat.photo?item.chat.photo:defaultImg} alt="conversation" />
                      <div style={{position: 'absolute', backgroundColor: session_chat_status && session_chat_status.hasOwnProperty(item.chat.user_id) && session_chat_status[item.chat.user_id] === 'online'?'#58A847':'#efefef', borderRadius: '100%', width: '0.59rem', height: '0.59rem', right: '18%', top: '0%', border: '1px solid darkgrey'}}></div>
                  </Col>
                  <Col onClick={() => onSelectUser(item.chat)} style={{ paddingTop: '1%', paddingLeft: '2%', paddingRight: '0%' }} xs={4} sm={7} md={7} lg={7} xl={7}>
                    <Row style={{ padding: '0px', margin: '0px' }}>
                      <Col style={{ paddingLeft: '0px' }} xs={{ span: 12 }} sm={{ span: 7 }} md={{ span: 7 }} lg={{ span: 7 }} xl={{ span: 7 }}>
                        <h1 className='conversation-title'>{ item.chat.name }</h1>
                        {isTypingMsg && isTypingMsg.hasOwnProperty(item.chat.chat_id) && isTypingMsg[item.chat.chat_id]?
                        <Row>
                        <Col xs={4} sm={4} md={4} lg={4} xl={4}>
                          <span style={{ fontSize: '0.7rem', color: '#0A73F0', fontStyle:'italic' }}>typing</span>  
                        </Col>
                        <Col style={{ paddingLeft: '18px' }} xs={8} sm={8} md={8} lg={8} xl={8}>
                          <PulseLoader
                            css={override}
                            size={3}
                            color={"#0A73F0"}
                            loading={isTypingMsg && isTypingMsg.hasOwnProperty(item.chat.chat_id)?isTypingMsg[item.chat.chat_id]:false}
                          />
                        </Col>
                        </Row>
                        :
                        <p className="conversation-snippet"> { recent_msg && recent_msg.hasOwnProperty(item.chat.chat_id)?recent_msg[item.chat.chat_id]:'' } </p>
                      }
                      </Col>
                      <Col style={{ paddingLeft: '0px', paddingRight: '0px' }} xs={12} sm={5} md={5} lg={5} xl={5}>
                        <p style={{ float: 'left', marginBottom: '0px', paddingTop: '1%', paddingLeft: '0%', fontSize: '0.5rem', color: '#0578FA' }}>
                          {last_seen_state && last_seen_state.hasOwnProperty(item.chat.chat_id) && last_seen_state[item.chat.chat_id].hasOwnProperty(item.chat.user_id)? <Moment fromNow>{last_seen_state[item.chat.chat_id][item.chat.user_id]}</Moment>:''}
                        </p>
                      </Col>
                    </Row>
                  </Col>
                  <Col style={{ paddingTop: '1%', paddingLeft: '0%' }} xs={4} sm={2} md={2} lg={2} xl={2}>
                  <CustomBadge message_count={true} count={recentMsgCount[item.chat.chat_id]}/>
                    {/* <OverlayTrigger
                              key="bottom"
                              placement="top"
                              overlay={
              ...
              <Row className="text-center" style={{ fontSize: '0.8rem', fontWeight: 'bold', marginTop: '10%' }} >
